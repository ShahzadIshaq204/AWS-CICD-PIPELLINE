...
abstract class Controller extends BaseController {
	...
	public function __construct()
	{
        ...
        // Load Threedrenders Config
        $threedrenders 	= config('threedrenders');
        $this->config = $threedrenders ;
        $this->data['threedrendersconfig'] = $threedrenders ;  
	} 	
	...
	function getImport( Request $request)
	{
		...
		if($task !='')
		{
			...
			return view('threedrenders.module.utility.import', $this->data);
		}	
	...
	public function getExpotion()
	{
		...
		return view('threedrenders.module.utility.export', $this->data);
	}
	public function getExport( Request $request, $t = 'excel')
	{
		...
		if($t == 'word')
		{			
			 return view('threedrenders.module.utility.word',$content);
			 
		} else if($t == 'pdf') {	

			$pdf = PDF::loadView('threedrenders.module.utility.pdf', $content)->setPaper('a4', 'landscape');
			return $pdf->download('invoice.pdf');
			
		} else if($t == 'csv') {		
		 
			return view('threedrenders.module.utility.csv',$content);
			
		} else if ($t == 'print') {
		
			//return view('threedrenders.module.utility.print',$content);
			$data['html'] = view('threedrenders.module.utility.print', $content)->render();	
			return view('layouts.blank',$data);			 
			 				 
		} else  {			
		
			 return view('threedrenders.module.utility.excel',$content);
		}	
	}		
	...
	function getLookup( $request)
	{
		...
		if(count($args)>=2) 
		{
			...
			$data['key']		= $info['key'];
			//$data['ids']		= 'md'-$info['id'];
			return view('threedrenders.module.utility.masterdetail',$data);
