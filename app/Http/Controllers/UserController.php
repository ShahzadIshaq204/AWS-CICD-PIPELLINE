...
class UserController extends Controller {
	...
	public function getRegister() {
		...
		if(config('threedrenders.cnf_regist') =='false') :
			if(\Auth::check()):
	...
	public function postCreate( Request $request) {
		...
		if ($validator->passes()) {
			...
			if(config('threedrenders.cnf_activation') == 'confirmation')
			{ 
	...
	public function getLogin() {
		...
		if(\Auth::check())
		{
			return redirect('account')->with(['message'=>'success','Youre already login','status'=>'success']);
	...
	public function reCaptcha( $request)
	{
        ...
        {
            $api_url = 'https://www.google.com/recaptcha/api/siteverify?secret=' . config('threedrenders.cnf_recaptchaprivatekey') . '&response='.$request['g-recaptcha-response'];
       ...
       /// die;
		...
		if(config('threedrenders.cnf_recaptcha') =='true') {
			...
			if($return !== false)
			{
				if($return['success'] !='true')
				{
					return response()->json(['type' => $return['success'], 'message' =>'Invalid reCpatcha']);
				}
			...
			if (\Auth::attempt(array('email'=>$request->input('email'), 'password'=> $request->input('password') ), $remember )
				...
				if(\Auth::check())
				{
					...
					if($row->active =='Inactive')
					{
						...
						if($request->ajax() == true )
						{
							return response()->json(['type' => 'danger', 'message' => 'Your Account is not active']);
						} else {
							\Auth::logout();
					...
					} else if($row->active=='Banned')
					{
						...
						if($request->ajax() == true )
						{
							return response()->json(['type' => 'danger', 'message' => 'Your Account is BLocked']);
						} else {
							// BLocked users
							\Auth::logout();
						...
						if(!is_null($request->input('language')))
						{
							...
							$session['lang'] = config('threedrenders.cnf_lang');
						...
						if($request->ajax() == true )
						{
							...
							if( config('threedrenders.cnf_front') =='false') :
								return response()->json(['type' => 'success', 'url' => url('account')]);
							else :
								return response()->json(['type' => 'success', 'url' => url('account')]);
							endif;	
				...
				if($request->ajax() == true )
				{
					return response()->json(['type' => 'danger', 'message' => 'Your username/password combination was incorrect']);
				} else {
				...
				if($request->ajax() == true)
				{
					return response()->json(['type' => 'error', 'message' => 'The following  errors occurred']);
				} else {
	...
	public function postRequest( Request $request)
	{
		...
		if ($validator->passes()) {	
			...
			if($user->count() >=1)
			{
				...
				$subject = "[ " .config('threedrenders.cnf_appname')." ] REQUEST PASSWORD RESET ";
