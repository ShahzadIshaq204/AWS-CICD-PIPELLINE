...
class HomeController extends Controller {
	...
	public function index( Request $request) 
	{
		...
		if(config('threedrenders.cnf_front') =='false' && $request->segment(1) =='' ) :
			return redirect('dashboard');
		endif; 	

		//print_r( config('threedrenders'));
		...
		if($page !='') {
			...
			$row = $sql[0];
			if(file_exists(base_path().'/resources/views/layouts/'.config('threedrenders.cnf_theme').'/template/'.$row->filename.'.blade.php') && $row->filename !='')
			{
				$page_template = 'layouts.'.config('threedrenders.cnf_theme').'.template.'.$row->filename;
			} else {
				$page_template = 'layouts.'.config('threedrenders.cnf_theme').'.template.page';
			}	
			...
			if($row->template =='frontend'){
				$page = 'layouts.'.config('threedrenders.cnf_theme').'.index';
	...
	public function  getSkin($skin='threedrenders')
	{
		...
		if(count($rows))
		{
			...
			if($row->method =='email')
			{
				...
				$data = array(
					'email'		=> $row->email ,
					'content'	=> $content ,
					'subject'	=> "[ " .config('threedrenders.cnf_appname')." ] New Submited Form ",
					'title'		=> $row->name 			
				);
			
				if( config('threedrenders.cnf_mail') =='swift' )
				{ 				
					\Mail::send('threedrenders.form.email', $data, function ( $message ) use ( $data ) {
			    		$message->to($data['email'])->subject($data['subject']);
			    	});		

				}  else {

					$message 	 = view('threedrenders.form.email', $data);
	...
	public function posts( Request $request ,  $category = '') 
	{
		...
		$this->data['pages'] = 'layouts.'.config('threedrenders.cnf_theme').'.blog.index';
		$page = 'layouts.'.config('threedrenders.cnf_theme').'.index';
		return view( $page , $this->data);	
	}
	...
	//	print_r($posts);exit;
		...
		$data = [
			...
			'posts'	=> $row ,
			'comments'	=>  $comments ,
			'pages' => 'layouts.'.config('threedrenders.cnf_theme').'.blog.view',
