<?php namespace App\Http\Controllers\Threedrenders;

use App\Models\Threedrenders\Module;
use App\Library\ZipHelpers;
use App\Library\ThreedrendersHelpers;
...
class ModuleController extends Controller {
	...
	public function index( Request $request)
	{
		...
		return view('threedrenders.module.index',$this->data);
	}
    ...
    function getCreate()
    {
        ...
        return view('threedrenders.module.create',$this->data);
     ...
     //   print_r($request->all());exit;
        ...
        if ($validator->passes()) {
            ...
            if($request->input('creation') == 'manual')
            {
                if($where =="")                
                {
                    return redirect('threedrenders/module/create')
            ...
            if(count($row) >= 1)
            {
                ...
                return redirect('threedrenders/module/rebuild/'.$id->module_id);
            } else {
                return redirect('threedrenders/module');
            }
    ...
    function getDestroy( $id = null )
    {
        $row = \DB::table('tb_module')->where('module_id', $id)
                                ->get();        
        if(count($row) <= 0){
            return redirect('threedrenders/module')->with('message','Can not find module')->with('status','error');
    ...
    function getConfig( $id )
    {
        ...
        // Get config Info 
        $fp = base_path().'/resources/views/threedrenders/module/template/'.$this->getTemplateName($row->module_type).'/config/info.json';
		...
		return view('threedrenders.module.config',$this->data);
        ...
        if ($validator->passes()) {
            ...
            if($request->ajax() == true)
            {
                return response()->json(array('status'=>'success','message'=>'Module Info Has Been updated  Successfull')); 
                 ...
                 return redirect('threedrenders/module/config/'.$request->input('module_name'))
    ...
    public function postSavesetting( Request $request)
    {
        ...
        if($request->ajax() == true)
        {
            return response()->json(array('status'=>'success','message'=>'Module Setting Has Been Save Successfull')); 
             ...
             return redirect('threedrenders/module/config/'.$row->module_name)
    ...
    function getSql(Request $request , $id )
    {
		...
		return view('threedrenders.module.sql',$this->data);
    ...
    function postSavesql( Request $request ,$id )
    {
        ...
        try {   
            ...
            // Do something when query fails. 
            $error ='Error : '.$select .' '.$where.' '.$group ;
            return redirect('threedrenders/module/sql/'.$request->input('module_name'))
       ...
       \DB::table('tb_module')
        ...
        if($request->ajax() == true)
        {
            return response()->json(array('status'=>'success','message'=>'SQL Has Changed Successful.')); 
             ...
             return redirect('threedrenders/module/sql/'.$row->module_name)
    ...
    function getTable( $id )
    {
		...
		$this->data['module_name'] = $row->module_name;
        $this->data['type']     = ($row->module_type =='ajax' ? 'addon' : $row->module_type);  
		return view('threedrenders.module.table',$this->data);
    ...
    function getForm( $id )
    {
		...
		$this->data['module_name'] = $row->module_name;
        $this->data['type']     = ($row->module_type =='ajax' ? 'addon' : $row->module_type);  
		return view('threedrenders.module.form',$this->data);
    ...
    public function postSaveform( Request $request)
    {
        ...
        if($request->ajax() == true)
        {
            ...
            return redirect('threedrenders/module/form/'.$row->module_name)
    ...
    public function getEditform(Request $request, $id )
    {
        ...
        $this->data['module'] = 'module';
        $this->data['module_name'] = $row->module_name;
        return view('threedrenders.module.field',$this->data);
    } 
    ...
    public function postSavetable( Request $request)
    {
        ...
        if($request->ajax() == true)
        {
            return response()->json(array('status'=>'success','message'=>'Module Table Has Been Save Successfull')); 
             ...
             return redirect('threedrenders/module/table/'.$row->module_name)
   ...
   function getPermission( $id )
    {
        ...
        */   
        ...
        $this->data['type']     = ($row->module_type =='ajax' ? 'addon' : $row->module_type);  
        return view('threedrenders.module.permission',$this->data);
        ...
        */         
        ...
        if($request->ajax() == true)
        {
            return response()->json(array('status'=>'success','message'=>'Permission Has Changed Successful')); 
             ...
             return redirect('threedrenders/module/permission/'.$row->module_name)
    ...
    function getBuild( $id )
    {
        ...
        return view('threedrenders.module.build',$this->data);
    ...
    function getFormdesign( Request $request , $id)
    {
       ...
       // echo '<pre>'; print_r($config); echo '</pre>';  exit;
        ...
        $this->data['module_name'] = $row->module_name;
        $this->data['type']           = $row->module_type;
        return view('threedrenders.module.formdesign',$this->data);
    }   
    ...
    public function postFormdesign( Request $request)
    {
      ...
      \DB::table('tb_module')
        ...
        return redirect('threedrenders/module/formdesign/'.$row->module_name)
    ...
    function getSub( Request $request, $id ='')
    {
		...
		return view('threedrenders.module.sub',$this->data);
    ...
    function postSavesub( Request $request)
    {
        ...
        if ($validator->passes()) {                
            ...
            if($request->ajax() == true)
            {
                ...
                return redirect('threedrenders/module/sub/'.$row->module_name)
                ->with('message','Master Has beed added Successful.')->with('status','success');   
            } 

        }    else {

            return redirect('threedrenders/module/sub/'.$request->get('module_name'))
            ->with('message', 'The following errors occurred')->with('status','error')
            ->withErrors($validator)->withInput();
    ...
    function getConn( Request $request , $id )
    {
        ...
        return view('threedrenders.module.connection',$this->data);
    }   
   ...
   function configGrid ( $field , $alias , $type, $sort ) {
    ...
    function postDobuild( Request $request ,$id )
    {
        ...
        $url = 'threedrenders/module/rebuild/'.$id."?rebuild=y&c={$c}&m={$m}&g={$g}&f={$f}&v={$v}&fg={$fg}&fv={$fv}&ff={$ff}";
    ...
    public function getRebuild(Request $request, $id = 0)
    {
           ...
           $mType = ( $row->module_type =='addon' ? 'native' :  $row->module_type);
            if(is_dir( base_path().'/resources/views/threedrenders/module/template/'.$mType ))
            {
                 require_once( base_path().'/resources/views/threedrenders/module/template/'.$mType.'/config/config.php');
            }  else {
                ...
                if($request->ajax() == true && \Auth::check() == true)
                {
                    ...
                    return redirect('threedrenders/module')->with('message','Template is Not Exists')
                            ->with('status','success');   
                }  
            }           
            self::createRouters();
        ...
        if($request->ajax() == true && \Auth::check() == true)
        {
            ...
            return redirect('threedrenders/module')->with('message','Code Script has been replaced successfull')->with('status','success');
        }  
    }  
       ...
       foreach($rows as $row)
        {
           ...
           $mType = ( $row->module_type =='addon' ? 'native' :  $row->module_type);
           include(base_path().'/resources/views/threedrenders/module/template/'.$mType.'/config/route.php' );
           include(base_path().'/resources/views/threedrenders/module/template/'.$mType.'/config/route_api.php' );
    ...
    function postPackage( Request $request) 
    {
       ...
       // echo '<pre>';print_r($this->data); echo '</pre>'; exit;
        return view('threedrenders.module.package',$this->data);
  ...
  function postInstall( Request $request ,$id =0)
  {
    ...
    if ($validator->passes()) {
      ...
      $data = \ThreedrendersHelpers::cf_unpackage($path);
      ...
      self::createRouters();
          ...
          return redirect('threedrenders/module')->with('message','Module Installed' . $msg)->with('status','success');
    }  else  {
         return redirect('threedrenders/module')->with('message','Please select file to upload !')->with('status','error');
    } 
  ...
  function getSubform( Request $request ,$id =0)
  {
      ...
      //  print_r($this->data['subform']);
        ...
        return view('threedrenders.module.subform',$this->data);
  }
   ...
   function postSavesubform( Request $request)
    {
        ...
        if ($validator->passes()) {                
            ...
            $row = \DB::table('tb_module')->where('module_id', $id)
                                    ->get();
            if(count($row) <= 0){
                return redirect('threedrenders/module/subform/'.$request->get('module_name'))
                    ->with('message', 'Can not find module')->with('status','error');      
            }
            $row = $row[0];                                    
            $this->data['row'] = $row;    
            ...
            */
            ...
            if($request->ajax() == true)
            {
                ...
                return redirect('threedrenders/module/subform/'.$row->module_name)
    ...
    function getSource( Request $request ,$id)
    {
        ...
        return view('threedrenders.module.source',$this->data);
...
//       echo '<pre>'; print_r($config); echo '</pre>'; 
        ...
        return view('threedrenders.module.duplicate',$this->data);
        ...
        if ($validator->passes()) {
            ...
            if(count($res)>=1)
            {
               ...
               // echo $row->module_id ; exit;
                ...
                return redirect('threedrenders/module/rebuild/'.$row->module_id.'?mode=duplicate');
         
            } else {
                return redirect('threedrenders/module')->with('message','Failed to Duplicate Module !' )->with('status','error');
