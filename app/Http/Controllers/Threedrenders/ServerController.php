...
class ServerController extends Controller {
    ...
    public function __construct()
    {
        ...
        $this->threedrenders_server = config('version')['Repository']  ;    
    }

    function index()
    {
        $this->data['version'] = config('version');
        return view('threedrenders.server.index', $this->data);
    }
    function show( Request $request , $task )
    {
        switch ($task) {
            case 'repository':
                 return view('threedrenders.server.repository', $this->data);
                break;
            case 'install':
                $this->data['id'] = $request->input('id');
                $this->data['type'] = $request->input('t');
                 return view('threedrenders.server.authen', $this->data);
                break;    
             case 'load':
                $page = $request->input('page') ;
                $page = ( $page =='' ? '' : '&page='.$page);
                $getRows = file_get_contents($this->threedrenders_server.'repos' ) or die ('ERROR');
                $rows = json_decode( $getRows   );
               // echo '<pre>'; print_r($rows); echo '</pre>'; exit;
                $this->data['rows'] =  $rows->data ;
               
                return view('threedrenders.server.item', $this->data);
                break;           
            default:
    ...
    public function doInstall( $request) 
    {
        ...
        $message =  '';
      //  return 'http://threedrenders5.net/api/download?id='.$id.'&uname='.$uname;
        ...
        $newInstall = file_get_contents( $this->threedrenders_server .'repos/download?id='.$id) or die ('ERROR');
        ...
        if( is_array($test) && count($test) >=1 ){
            ...
            if($type =='' or $type =='module') {
                $data = \ThreedrendersHelpers::cf_unpackage( $temp );
                $msg = '.';
                ...
                \ThreedrendersHelpers::upgrade( $temp);
                unlink($temp);               
            }  
    ...
    public function doUpdate( $request) {
           ...
           $path = base_path()."/resources/views/threedrenders/server/version.json";
            ...
            if ( !is_file(  $temp )) 
            {
                ...
                $newUpdate = file_get_contents($this->threedrenders_server . 'repos/upgrade');
                $test = json_decode($newUpdate,true);
                 if( is_array($test) && count($test) >=1 ){
                    ...
                    if(\ThreedrendersHelpers::upgrade( $temp))
                    {
    ...
    public function version( Request $request ) {
        ...
        //if($check){            
            ...
            //Check For An Update
            $getVersions = file_get_contents( $this->threedrenders_server .'repos/changelog?codename='.$curr_version['Codename']) or die ('ERROR');
       ...
       foreach($rows as $row)
        {
           ...
           include(base_path().'/resources/views/threedrenders/module/template/'.$row->module_type.'/config/route.php' );
