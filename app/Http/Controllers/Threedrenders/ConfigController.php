<?php namespace App\Http\Controllers\threedrenders;
...
class ConfigController extends Controller {
	...
	{	
        ...
        $this->data['pageNote']  =   __('core.t_generalsettingsmall');	
		$this->data['active'] = '';
		return view('threedrenders.config.index',$this->data);	
	}
...
$val  .= 	"];\n";
			...
			$filename = base_path().'/config/threedrenders.php';
			//chmod(  $filename , 775);
			$fp=fopen($filename,"w+"); 
			fwrite($fp,$val); 
			fclose($fp); 

			return redirect('threedrenders/config')->with('message','Setting Has Been Save Successful')->with('status','success');
		} else {
			return redirect('threedrenders/config')->with('message', 'The following errors occurred')->with('status','success')
	...
	public function getEmail()
	{
		...
		return view('threedrenders.config.email',$this->data);		
	...
	function postEmail( Request $request)
	{
		...
		if ($validator->passes()) 
		{
			...
			return redirect('threedrenders/config/email')->with('message', 'Email Has Been Updated')->with('status','success');	
			
		}	else {

			return redirect('threedrenders/config/email')->with('message', 'The following errors occurred')->with('status','success')
			->withErrors($validator)->withInput();
		}
	...
	public function getSecurity()
	{
		...
		return view('threedrenders.config.security',$this->data);		
...
$val  .= 	"];\n";
			...
			fwrite($fp,$val); 
			fclose($fp); 
			return redirect('threedrenders/config/security')->with('message','Setting Has Been Save Successful')->with('status','success');
		} else {
			return redirect('threedrenders/config/security')->with('message', 'The following errors occurred')->with('status','error')
			->withErrors($validator)->withInput();
		}	
	}
	...
	public function getLog( $type = null)
	{
		...
		return view('threedrenders.config.log',$this->data);	
	}
	...
	public function getClearlog()
	{
		...
		//return redirect('threedrenders/config/log')->with('message','Cache has been cleared !')->with('status','success');	
	}
	...
	public function getTranslation( Request $request, $type = null)
	{
		...
		return view('threedrenders.config.translation.'.$template,$this->data);	
	}
	
	public function getAddtranslation()
	{
		return view("threedrenders.config.translation.create");
	} 
	...
	public function postAddtranslation( Request $request)
	{
		...
		if ($validator->passes()) {
			...
			return redirect('threedrenders/config/translation')->with('messagetect','New Translation has been added !')->with('status','success');	;			
			
		} else {
			return redirect('threedrenders/config/translation')->with('message','Failed to add translation !' )->with('status','error')->withErrors($validator)->withInput();
		}		
	...
	//	$filename = 'lang.php';
		...
		fwrite($fp,$form); 
		fclose($fp); 	
		return redirect('threedrenders/config/translation?edit='.$lang.'&file='.$file)
	...
	public function getRemovetranslation( $folder )
	{
		...
		return redirect('threedrenders/config/translation')->with('message','Translation has been removed !')->with('status','success');	
