...
class SiteHelpers
{
                ...
                if($form['view'] != 0)
                {
                    if($form['field'] !='entry_by')
                    {
                        if(isset($form['option']['tooltip']) && $form['option']['tooltip'] !='')
                            $tooltip = '<a href="javascript:void(0);" data-toggle="tooltip" placement="left" class="tips" title="'. $form['option']['tooltip'] .'"><i class="icon-question2"></i></a>';
    ...
    public static function toMasterDetail( $info )
    {
        ...
        if(count($info)>=1)
        {
	 	...
	 	\$this->data['subform'] = \$this->detailview(\$this->modelview ,  \$this->info['config']['subform'] ,\$id );";
            ...
            $tpl = array();
            require_once('../resources/views/threedrenders/module/template/native/masterdetailform.php');
    ...
    public static function showMaps( $val ){
        ...
        return '<iframe
			  ...
			  src="https://www.google.com/maps/embed/v1/place?key='.config("threedrenders.cnf_maps").'&q='.$lang.','.$long.'" allowfullscreen>
			</iframe>';
    ...
    public static function showFormMaps( $val , $id  ){
        ...
        require_once('../resources/views/threedrenders/module/utility/maps.php');
    }
    ...
    public static function formatLookUp($val , $field, $arr )
    {
        ...
        if(isset($arr['0']) && $arr['0'] ==1)
        {
            ...
            if(!empty($val))
                $Q = DB::select(" SELECT ".str_replace("|",",",$arr['3'])." FROM ".$arr['1']." WHERE ".$arr['2']." IN (".$val.") ");
    ...
    {
        $threedrendersconfig = config('threedrenders');
        ...
        if($format_as =='image')
        {
            ...
            if($format_value =='')
            {
                if($threedrendersconfig['cnf_date'] !='' )
                {
                    $value = date("".$threedrendersconfig['cnf_date']."",strtotime($value));
                }
            } else {
    ...
    {
        $path = base_path().'/resources/views/threedrenders/module/template/';
        $fld = scandir($path);
    ...
    public static function BBCode2Html($text) {
        ...
        $emotion =  url('threedrenders/js/plugins/markitup/images/emoticons/');
    ...
    static public function buttonAction( $module , $access , $id , $setting)
    {
        ...
        if($access['is_detail'] ==1) {
            if($setting['view-method'] != 'expand')
            {
                ...
                if($setting['view-method'] =='modal')
                    $onclick = " onclick=\"ThreedrendersModal(this.href,'View Detail'); return false; \"" ;
        ...
        if($access['is_edit'] ==1) {
            ...
            if($setting['form-method'] =='modal')
                $onclick = " onclick=\"ThreedrendersModal(this.href,'Edit Form'); return false; \"" ;
    ...
    static public function buttonActionCreate( $module  ,$method = 'newpage')
    {
        ...
        if($method['form-method'] =='modal')
            $onclick = " onclick=\"ThreedrendersModal(this.href,'Create Detail'); return false; \"" ;
    ...
    static public function htmlExpandGrid()
    {
        ...
        return View::make('threedrenders.module.utility.extendgrid');
    }
    ...
    public static function getCUser(){
        if(\Session::get('uid')==null) return array();
        $u=\DB::table('tb_users')
    ...
    public static function bulkReplace($array, $string){
