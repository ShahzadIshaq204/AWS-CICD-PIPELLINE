...
<div class="page-header"><h2> {{ $pageTitle }} <small> {{ $pageNote }} </small></h2></div>
						...
						<div class="btn-group">
					        ...
					        <ul class="dropdown-menu">
					         ...
					         @if($access['is_remove'] ==1)
								 <li class="nav-item"><a href="javascript://ajax"  onclick="ThreedrendersDelete();" class="nav-link tips" title="{{ __('core.btn_remove') }}"><i class="fa fa-trash-o"></i>
			...
			<div class="table-responsive" >
 			{!! Form::open(array('url'=>'notification?'.$return, 'class'=>'form-horizontal m-t' ,'id' =>'ThreedrendersTable' )) !!}
		    ...
		    <table class="table table-hover " id="{{ $pageModule }}Table">
		        ...
		        <tbody>        						
		            @foreach ($rowData as $row)
		                <tr>
							<td > {{ ++$i }} </td>
							<td ><input type="checkbox" class="ids minimal-green" name="ids[]" value="{{ $row->id }}" value="{{ $row->id }}" />  <label for="v-{{ $row->id }}"> </label></td>
							<td>
