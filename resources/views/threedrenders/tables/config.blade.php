...
{!! Form::open(array('url'=>$action, 'class'=>'form-horizontal','id'=>'saveform' , 'parsley-validate'=>'','novalidate'=>' ')) !!}
				...
				@foreach($columns as $column)
					<tr class="" id="field-{{ $column->Field }}">
						...
						<td>
							...
							<a href="{{ URL::TO('threedrenders/tables/tablefieldedit/'.$table.'?'.$info) }}" class="btn btn-sm" onclick="ThreedrendersModal(this.href,'Edit New Column : {{$column->Field}} '); return false;" ><i class="ti-pencil "></i></a>
							<a href="{{ URL::TO('threedrenders/tables/tablefieldremove/'.$table.'/'.$column->Field) }}" rel="#field-{{ $column->Field }}" class=" btn btn-smremovedField  text-danger"><i class="fa fa-trash"></i></a>
						</td>
		...
		@else
		<a href="{{ URL::TO('threedrenders/tables/tablefieldedit/'.$table) }}" class="btn btn-xs btn-info" onclick="ThreedrendersModal(this.href,'Add New Column'); return false;" > Add New Field</a>
		@endif	
...
function showResponse(data)  {		
	...
	{
		window.location.href = '{{ url("threedrenders/tables") }}';
