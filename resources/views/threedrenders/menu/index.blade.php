...
<div class="toolbar-nav">
    <div class="btn-group">
    	<a href="{{ url('threedrenders/menu/index/0?pos=top')}}" class="btn  btn-sm btn-primary"> <i class="fa fa-plus"></i> new  </a>
        <a href="{{ url('threedrenders/menu?pos=top')}}" class="btn  btn-sm btn-info"> <i class="fa fa-bars"></i> {{ __('core.tab_topmenu') }}  </a>
        <a href="{{  url('threedrenders/menu?pos=sidebar') }}" class="btn  btn-sm btn-success post_url"> <i class="fa fa-bars"></i> {{ __('core.tab_sidemenu') }} </a>
    </div>
</div>
			...
			<div class="sbox-content">
					...
					<fieldset style="min-height: 400px;">
						...
						<div id="list2" class="dd myadmin-dd-empty " style="min-height:350px;">
							...
							@foreach ($menus as $menu)
								  <li data-id="{{$menu['menu_id']}}" class="dd-item dd3-item">
									<div class="dd-handle dd3-handle"></div><div class="dd3-content">{{$menu['menu_name']}}
										...
										<a href="{{ url('threedrenders/menu/index/'.$menu['menu_id'].'?pos='.$active)}}"><i class="ti-pencil"></i></a></span>
									</div>
											...
											@foreach ($menu['childs'] as $menu2)
											 <li data-id="{{$menu2['menu_id']}}" class="dd-item dd3-item">
												<div class="dd-handle dd3-handle"></div><div class="dd3-content">{{$menu2['menu_name']}}
													<span class="pull-right">
													<a href="{{ url('threedrenders/menu/index/'.$menu2['menu_id'].'?pos='.$active)}}"><i class="ti-pencil"></i></a></span>
												</div>
													...
													@foreach($menu2['childs'] as $menu3)
													 	<li data-id="{{$menu3['menu_id']}}" class="dd-item dd3-item">
															<div class="dd-handle dd3-handle"></div><div class="dd3-content">{{ $menu3['menu_name'] }}
																<span class="pull-right">
																<a href="{{ url('threedrenders/menu/index/'.$menu3['menu_id'].'?pos='.$active)}}"><i class="ti-pencil"></i></a>
																</span>
															</div>
														</li>	
													@endforeach
							...
							@endforeach			  
							 ...
							 {!! Form::open(array('url'=>'threedrenders/menu/saveorder', 'class'=>'form-horizontal','files' => true)) !!}
				...
				<div class="col-md-7" >
					<fieldset style="min-height: 400px;">
		 			...
		 			{!! Form::open(array('url'=>'threedrenders/menu/save', 'class'=>'form-horizontal','files' => true  , 'parsley-validate'=>'','novalidate'=>' ')) !!}
						  ...
						  <div class="form-group row  " >
							...
							<div class="col-md-8">
							  ...
							  <p> View Icon Codes : 
							  <a href="{{ url('threedrenders/menu/icon')}}" onclick="ThreedrendersModal(this.href,'Select Icon'); return false;"> Browse Icons  </a>
							 </div> 
						  </div> 					
						  <div class="form-group row  " >
					  ...
					  <div class="form-group row">
						...
						@if($row['menu_id'] !='')
							<button type="button"onclick="ThreedrendersConfirmDelete('{{ url('threedrenders/menu/destroy/'.$row['menu_id'].'?pos='.$active)}}')" class="btn btn-danger btn-sm">  Delete </button>
						@endif	
						</div>	  
